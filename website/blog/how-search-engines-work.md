...
_Originally published in 2023; technical corrections reviewed September 25, 2026. The Tantivy examples explain the original architecture. Summa 2 has its own Rust engine; see also [dense retrieval](dense-retrieval.md) and [sparse retrieval](sparse-retrieval.md)._
...
**Relevance** means how well a document satisfies the user's information need. Semantic proximity is one useful signal, but an exact identifier, a date constraint, or the right answer can matter more than topical similarity. In mathematical language, proximity can be expressed through the proximity of vectors. Therefore, for the mathematical expression of relevance, it is necessary to associate vectors in some vector space with documents and queries from the world of people.
...
An exact scan has to score every vector, which becomes expensive as the collection grows. Approximate nearest-neighbor indexes reduce this work by exchanging some recall for speed; dense retrieval is now a practical part of large search systems. Here we will follow a different route and study lexical scoring, whose additive structure lets us use an inverted index. The two approaches can also be combined.
...
Firstly, saturation reduces the advantage of repeating one term many times. It does not guarantee that a document matching every query term outranks one matching fewer terms: IDF, document length and boosts still matter.
...
$$\frac{tf(t, d)}{tf(t, d) + k(1 - b + b\frac{dl(d)}{dl_{avg}})}$$

When $$b = 0$$, the formula may be simplified into $$\frac{tf(t, d)}{tf(t, d) + k}$$, and when $$b = 1$$, the formula takes the form

$$\frac{tf(t, d)}{tf(t, d) + k\frac{dl(d)}{dl_{avg}}}$$
...
We have obtained a BM25-style score. The original Robertson–Sparck Jones form of IDF uses $$\log\frac{N-df(t)+0.5}{df(t)+0.5}$$, which can be negative for very common terms. Modern Lucene uses the nonnegative variant

$$IDF(t)=\log\left(1+\frac{N-df(t)+0.5}{df(t)+0.5}\right).$$

The extra 1 matters when reasoning about nonnegative score contributions and pruning bounds. See [Lucene's BM25 definition](https://lucene.apache.org/core/10_3_1/core/org/apache/lucene/search/similarities/BM25Similarity.html). Score scaling and encoded length norms also depend on the implementation, so identical parameter names do not by themselves promise byte-identical scores.
...
A dictionary implementation used in the original Tantivy example is the <a href="https://blog.burntsushi.net/transducers/">finite-state transducer</a>, available through the <a href="https://docs.rs/fst/0.4.5/fst/">fst</a> crate. Dictionary implementations and defaults vary by engine version. Simplifying it, prefix trees organize the dictionary by extracting common prefixes of the keys, while transducers can also extract common suffixes. Thus, the compression of the dictionary is performed more efficiently, but in the end, it becomes an acyclic graph instead of a tree.
...
Another option for storing the dictionary is SSTable. It is just a delta-compressed blocks of keys, possibly with secondary indices. Neither structure is universally faster. A blocked SSTable can trade additional decoding for fewer random reads, while an FST can benefit from compact shared structure. This property may be especially beneficial in network-mapped dictionaries.
...
$$score(q, d)=\sum_{t \in q} \frac{tf(t, d) (k + 1)}{tf(t, d) + k(1 - b + b\frac{dl(d)}{dl_{avg}})} \cdot IDF(t)$$
...
- $$dl_{avg}$$ - the total indexed token count for the field divided by the number of documents containing that field. Posting-list entry counts alone are insufficient because one entry may represent many occurrences
...
- Direct indexes, which allow extracting documents by its <i>DID</i>. Retrieval may require decompressing a stored-document block; codecs and block sizes vary. Current Tantivy supports LZ4, Zstd or uncompressed storage, as listed in its [feature documentation](https://github.com/quickwit-oss/tantivy)
...
The essence of the optimization is to split the query terms into two non-intersecting sets: "essential" and "nonessential". Documents that contain terms only from the "nonessential" set cannot enter the top-K and therefore their posting lists can be skipped forward to the first document that contains at least one "essential" term.

Remember $$UB_t$$ from the BM25 section? It must bound the largest possible contribution of the term under the actual scorer. IDF helps determine it, but term-frequency saturation, boosts and length normalization also matter. A convenient estimate is useful only if it cannot underestimate a document's contribution.

Having the importance of terms at hand, one can sort the terms in increasing order of their upper bounds and calculate the partial sums of importance from the first to the last term. All terms with a partial sum less than the current $$\theta$$ (the current minimum score from the top-K heap) can be assigned to the "nonessential" set. A document containing only terms from the "nonessential" set cannot be rated higher than the sum of the importance of these terms and therefore cannot be rated higher than $$\theta$$. Such documents will not be included in the final set.

Makes sense to consider documents that contain at least one term from the "essential" set of terms only. Therefore, we can skip the posting lists of "nonessential" terms until the smallest of the <i>DID</i>s that the "essential" term iterators point to. Without skip-lists we would have to run through the posting lists sequentially and there would be no big gain in speed.
...
As the heap threshold rises, the partition can be updated. Nonessential terms still contribute to the full score of a candidate nominated by an essential term; they are not removed from the scoring function. Equality with the threshold needs the collector's tie-breaking rule. Exact hit counts and aggregations may also require work that ranked top-K pruning alone would skip.
...
3. The <i>pivotTerm</i> is selected - the first term whose partial sum reaches the competitive threshold (including score ties that may enter the heap)
4. All previous iterators to the <i>pivotTerm</i> are checked.
   ...
   If at least one of the iterators points to a <i>DID</i> less than <i>pivotTerm.DID</i>, then such an iterator is advanced to the first <i>DID</i> greater than or equal to <i>pivotTerm.DID</i>.
   After that, we return to the first step of the algorithm

**Block-max WAND (BMW)**

BMW is an extension of the WAND algorithm from the previous section, proposed in <a href="https://doi.org/10.1145/2009916.2010048">Ding, 2011</a>. Instead of using global $$UB_t$$ for each term, we now split the posting lists into blocks and store $$UB$$ separately for each block. The algorithm repeats WAND, but also checks the partial sum of $$UB$$ of the blocks that the iterators are currently pointing to. If a conservative sum is below $$\theta$$, the covered document range can be skipped. Lists need not have aligned block boundaries: the bound is valid only over the range covered by all the contributing block summaries.
...
First, the query syntax tree is built. The parser must preserve meaningful syntax such as quotes, field qualifiers, operators and escaped literals. Text within query clauses is analyzed using the relevant field's tokenizer; lowercasing, stemming, lemmatization and stop-word removal depend on that configuration. Discarding punctuation before parsing would lose some of the query's meaning.
...
Immutability is a design choice that simplifies concurrent readers, compression, caching and publication. New documents go into new segments, while deletes can be recorded separately. Mutable index designs also exist; the point is to avoid in-place changes to the structures used by an existing reader.
...
Modern systems commonly combine RAM-resident metadata and caches with SSD or object-store payloads. The whole index does not have to fit in RAM: [DiskANN](https://www.microsoft.com/en-us/research/?p=634449) is one example for vectors, and [Quickwit](https://github.com/quickwit-oss/quickwit) uses object storage for search. Storage latency, locality and cache behavior still matter, even when the hardware is very different from the early Google design.
